...
GoogleSignin.configure({
    ...
    webClientId: "448393819239-s9q3t5pngpbao0pl4nm2eu5ae5ojsa33.apps.googleusercontent.com",
    offlineAccess: true
});
...
const SignInView = ({ navigation }) => {
        ...
        if (valid) {
            ...
            try {
                ...
                const { data } = await apiCall
                    ('POST', ENDPOINTS.USER_SIGN_IN, params);
                if (data.status === 200) {
                ...
                AccessToken.getCurrentAccessToken().then(
                    (data) => {
                        ...
                        const responseCallback = ((error, result) => {
                            console.log('accessTokenFaceBook: ', accessTokenFaceBook, ' : Profile: ', result)
                            var fbResponse = {
                                // login_type: 1,//define business or personal
    ...
    const _handleSocialLogin = async (params) => {
        ...
        try {
            const { data } = await apiCall
                ('POST', ENDPOINTS.SOCIAL_LOGIN, prms);
            if (data.status === 200) {
