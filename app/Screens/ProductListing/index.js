...
const ProductListing = ({ navigation, route }) => {
    ...
    const handleProductList = async (detail) => {
        ...
        try {
            ...
            const { data } = await apiCall("POST", ENDPOINTS.GET_PRODUCT_LIST, params)
            if (data.status === 200) {
                setVisible(false);
    ...
    const _renderProductList = (item, index) => {
        const setaddbtn = _.filter(shoppingCartData, { product_id: item.product_id });
            ...
            <View
                ...
                <View style={{ paddingTop: 5 }}>
                    ...
                    <View style={{ flexDirection: "row", }}>
                        ...
                        <View>
                            <Text style={[styles.text, { marginLeft: 20, color: YELLOW_COLOR_CODE }]}>
                                {item.price}
                            </Text>
                            <View
                                style={{
                    ...
                    {selected_row ?
                        ...
                        <Button
                            ...
                            style={{ padding: 0, paddingVertical: 5, width: "100%", marginVertical: 5 }}
                            buttonLabelStyle={{ fontSize: 14, fontFamily: FONT_FAMILY_REGULAR }}
                            buttonText='Add'
                        />
                    }
                </View>
