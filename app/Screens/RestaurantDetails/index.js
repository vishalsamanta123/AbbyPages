...
import { Dimensions, View, Text, Image, TouchableOpacity, Linking } from 'react-native';
import {
    GREY_COLOR_CODE,
    FONT_FAMILY_REGULAR,
...
const RestaurantDetailsView = ({ navigation, route }) => {
    ...
    const _handleOptions = (item, index) => {
        const selectedColor = index === isSelected ? BLACK_COLOR_CODE : GREY_COLOR_CODE
        return (
            <TouchableOpacity onPress={() => _handleDataTypeSelected(index, item)} style={styles.OptionsContain}>
    ...
    const _handlePopularDish = (item) => {
        return (
            <View style={styles.PopularConatiner}>
                <Image style={styles.PopularDishImg} resizeMode='contain' source={{ uri: item.image }} />
                <View style={{ padding: 10 }}>
                    <Text numberOfLines={1} style={styles.DishNameTxt}>{item.item_name}</Text>
            ...
            <Fragment>
                <View style={{
                    ...
                    <View style={styles.ReviewContainer}>
                        ...
                        {item.profile_image ?
                            <Image style={{ width: 80, height: 75, borderRadius: 75 }}
                        ...
                        <View style={{ paddingLeft: 20, width: '70%' }}>
                            <Text style={{ fontFamily: FONT_FAMILY_REGULAR, fontSize: 19 }}>{item.title}</Text>
                            <View style={{ flexDirection: 'row' }}>
                                <Text style={{ fontFamily: FONT_FAMILY_REGULAR, color: GREY_COLOR_CODE, fontSize: 13 }}>by  </Text>
                                <Text style={{ fontFamily: FONT_FAMILY_REGULAR, color: YELLOW_COLOR_CODE, fontSize: 13 }}>{item.first_name + item.last_name}</Text>
                                <Text style={{ fontFamily: FONT_FAMILY_REGULAR, color: GREY_COLOR_CODE, fontSize: 13 }}> |
                                    {moment(item.create_date).endOf('day').fromNow()}
                                </Text>
                            </View>
                            <View style={{ flexDirection: 'row', paddingTop: 5 }}>
                                <View style={styles.RatingStyles}><Text style={styles.RatingStylesTxt}>
                                    {
                    ...
                    <View style={{ paddingTop: 10 }}>
                        <Text style={{ color: GREY_COLOR_CODE, fontFamily: FONT_FAMILY_REGULAR, fontSize: 14 }}>
                            {item.description}
                        </Text>
                    </View>
                </View>
    ...
    const onPressReservation = (data, booking_type) => {
        navigation.navigate('RestauranrtBooking', { detail: data, booking_type: booking_type });
    };
    ...
    const handleUploadImage = async (img) => {
        ...
        try {
            ...
            const { data } = await apiCall('POST', ENDPOINTS.ADD_BUSINESS_PHOTO_VIDEO,
                formdata,
                {
    ...
    const _handlePhotos = (item, index) => {
        return (
            <View key={index} style={styles.PopularConatiner}>
                <Image style={[styles.PopularDishImg, {
