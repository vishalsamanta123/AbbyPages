            ...
            <Fragment>
                <TouchableOpacity
                    ...
                    <View style={{ padding: 15 }}>
                        <Text numberOfLines={1} style={styles.DishNameTxt}>{item.business_name}</Text>
                        ...
                        <Text
                            numberOfLines={3}
                            style={[styles.MainCovidPara, { width: 160 }]}>{item.about_business}
                        </Text>
                    </View>
                </TouchableOpacity>
            </Fragment>
    ...
    const _handlePeopleSaying = (item) => {
        return (
            <View style={styles.UserContainer}>
                <Image style={styles.UserProfileStyle} source={{ uri: item.profile_image }} />
                <View style={{ paddingLeft: 10, width: '70%' }}>
                    <Text style={[styles.ReviewFullView, { lineHeight: 19, paddingTop: 10 }]}>{item.description}</Text>
                </View>
            </View>
        )
    };
    ...
    const onPressFindTable = async () => {
        ...
        try {
            ...
            const { data } = await apiCall
                ('POST', ENDPOINTS.RESTAURANT_TIME_SLOAT, params);
            if (data.status === 200) {
                // setVisible(false);
                var currentDate = moment().format('YYYY-MM-DD');
                const searchArray = [...data.data]
                const filterData = _.filter(
                    searchArray, { date: currentDate });
