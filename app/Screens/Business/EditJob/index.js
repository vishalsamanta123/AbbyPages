...
const EditJob = ({ navigation, route }) => {
  ...
  useEffect(() => {
    setCompanyName(item?.company_name ? item?.company_name : "");
    setCompanyPersonName(
      item?.contact_person_name ? item?.contact_person_name : ""
    );
    setEmailID(item?.email ? item?.email : "");
    setInterviewDetails(item?.interview_details ? item?.interview_details : "");
    setJobAddress(item?.job_address ? item?.job_address : "");
    // setSelectedBenefits(item?.job_benefits ? item?.job_benefits : "");
    // setSelectedJobCategory(item?.job_category_id ? item?.job_category_id : "");
    setJobDescription(item?.job_description ? item?.job_description : "");
    setEndTimeDay(item?.job_end_time_day ? item?.job_end_time_day : "");
    setEndTime(item?.job_end_timing ? item?.job_end_timing : "");
    setJobLevel(item?.job_level ? item?.job_level : "");
    // setSelectedCountry(
    //   item?.job_location_country ? item?.job_location_country : ""
    // );
    // setSelectedState(item?.job_location_state ? item?.job_location_state : "");
    // setSelectedCity(item?.job_location_city ? item?.job_location_city : "");
    // setItemType();
    setJobReqiurements(item?.job_requirements ? item?.job_requirements : "");
    setStartTimeDay(item?.job_start_time_day ? item?.job_start_time_day : "");
    setStartTime(item?.job_start_timing ? item?.job_start_timing : "");
    setJobTimeings(item?.job_timing ? item?.job_timing : "");
    setJobTitle(item?.job_title ? item?.job_title : "");
    setLanguage(item?.language ? item?.language : "");
    setSalaryFrom(
      item?.monthly_in_hand_salary_from ? item?.monthly_in_hand_salary_from : ""
    );
    setSalaryTo(
      item?.monthly_in_hand_salary_to ? item?.monthly_in_hand_salary_to : ""
    );
