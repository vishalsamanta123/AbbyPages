...
const BasicInformation = ({ navigation }) => {
    ...
    const getUserData = async () => {
        ...
        try {
            const { data } = await apiCall
                ('POST', ENDPOINTS.GET_USER_BUSINESS_DETAILS);
            if (data.status === 200) {
    ...
    const GetCoutryList = async () => {
        try {
            ...
            const { data } = await apiCall
                ('POST', ENDPOINTS.COUNTRY_LIST, params);
            if (data.status === 200) {
    ...
    const onPressStepSecond = async () => {
        ...
        try {
            ...
            const { data } = await apiCall
                ('POST', ENDPOINTS.EDIT_BUSINESS_BASIC_INFORMATION, params);
            if (data.status === 200) {
                setVisible(false);
    ...
    async function onPressCity() {
        setCitySta(!CitySta)
        if (CitySta == false && BasicInformationData.country && BasicInformationData.state) {
            try {
                setVisible(true)
                const params = {
                    status: 2,
