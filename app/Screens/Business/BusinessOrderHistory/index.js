...
const BusinessOrderHistoryView = ({ route, props, navigation }) => {
    ...
    const [BusinessType, setBusinessType] = useState(OrderCategorySel.businsessType);
    ...
    const getBusinessTypeList = async (type) => {
        ...
        try {
            ...
            const { data } = await apiCall
                ('POST', ENDPOINTS.BUSINESS_CATEGORY_LIST, params);
            if (data.status === 200) {
    ...
    const getOrderListFun = async (businessType) => {
        ...
        try {
            ...
            const { data } = await apiCall
                ('POST', ENDPOINTS.GET_ORDER_LIST, params);
            if (data.status === 200) {
    ...
    const onpressButton = (item) => {
        if (item.business_type == 1) {
            item.order_booking_type == 1 ?
                navigation.navigate('FoodOrderDetails', { orderId: item.order_id, BusinessType })
                :
                item.order_booking_type == 3 ?
                    navigation.navigate('TableBookingDetails', { orderId: item.order_id, BusinessType })
                    :
                    item.order_booking_type == 4 ?
                        navigation.navigate('OutSideBookingOrderDetails', { orderId: item.order_id, BusinessType })
                        : null
        } else if (item.business_type == 3) {
            navigation.navigate('ServiceOrderDetails', { orderId: item.order_id, BusinessType })
        } else if (item.business_type == 2) {
            navigation.navigate('ShoppingOrderDetails', { orderId: item.order_id, BusinessType })

        }
    }
    const _handleOrders = (item, index) => {
        return (
            <TouchableOpacity style={styles.ConatinView} onPress={() => onpressButton(item)}>
                <Image
                ...
                <View style={styles.DishDiscptnView}>
                    <View >
                        {item.order_booking_type_text ?
                            <Text style={styles.DishNameTxt} >Order Type: {item.order_booking_type_text}</Text>
                            :
                    ...
                    {item.business_type == 1 ?
                        item.order_booking_type == 3 ?
                            <Text numberOfLines={1} style={styles.DiscrptnTxtStyle} >booking Id :- {item.order_id}</Text>
                            :
                            <Text numberOfLines={1} style={styles.DiscrptnTxtStyle} >Order Id :- {item.order_id}</Text>
                        :
                        item.business_type == 2 ?
                            <Text style={styles.DiscrptnTxtStyle}>Order Id :- {item.order_id}</Text>
                            :
                            null
                    }
                    {item.total_amount ?
                        <Text style={styles.DiscrptnTxtStyle} >Amount :- {item.total_amount}</Text>
                        :
                        null
                    }
                    <View style={styles.DateContainer}>
                        <Image style={styles.DateImge}
                            source={require('../../../Assets/list_calendar_icon.png')} />
                        <Text style={[styles.ReviewText, { paddingLeft: 10 }]}>{moment(item.create_order).format('MM/DD/YYYY')}</Text>
                    </View>
                    <View style={styles.ViewContainer}>
                        <View style={styles.PendingView}>
                            <Image style={styles.CheckImge}
                                source={require('../../../Assets/checked_circled_icon_box.png')}

                            />
                            <Text style={[styles.ReviewText, { paddingLeft: 10, color: YELLOW_COLOR_CODE }]}>
                                {item.order_status == 0 ? 'Pending' : item.order_status == 1 ? 'Confirmed' : item.order_status == 4 ? 'Canceled' : null}
                            </Text>
                        </View>
                        <TouchableOpacity
